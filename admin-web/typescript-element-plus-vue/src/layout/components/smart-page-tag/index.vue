...
function selectTab(val:any) {
  ...
  // 寻找tag
  let tag = tagNav.value.find((e: { menuName: any; }) => e.menuName == name);
  if (!tag) {
...
function closeByMenu(closeAll:boolean) {
  let find = tagNav.value.find((e: { menuName: RouteRecordName | null | undefined; }) => e.menuName == selectedKey.value);
  if (!find || closeAll) {
    closeTag(null, true);
  } else {
    closeTag(find, true);
...
function closeTag(item: UserTagNav | null, closeAll: boolean) {
  // 关闭单个tag
  if (item && !closeAll) {
    let goName = appDefaultConfig.homePageName;
    let goQuery = undefined;
    if (item.fromMenuName && tagNav.value.some((e: { menuName: string | undefined; }) => e.menuName == item.fromMenuName)) {
      goName = item.fromMenuName;
      goQuery = item.fromMenuQuery;
    } else {
      // 查询左侧tag
      let index = tagNav.value.findIndex((e: { menuName: string; }) => e.menuName == item.menuName);
      if (index > 0) {
        // 查询左侧tag
