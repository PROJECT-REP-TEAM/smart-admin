 ...
 * @LastEditTime: 2022-06-06 00:43:39
 * @LastEditors: LiHaiFan
 * @Description: 菜单新增编辑抽屉
 * @FilePath: \typescript-element-plus-vue\src\views\system\menu\components\menu-operate-modal.vue
-->
<template>
  <el-drawer :title="form.menuId ? '编辑' : '添加'" :size="900" v-model="visible" @close="onClose">
        ...
        <el-row :gutter="16">
          ...
          <el-col :span="12">
            <el-form-item label="排序" name="sort">
              <el-input-number style="width: 100%" placeholder="请输入排序" v-model="form.sort" :min="0" :max="9999" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-form-item v-if="form.menuType == MENU_TYPE_ENUM.MENU.value" label="组件地址" name="component">
          <el-input v-model="form.component" placeholder="请输入组件地址 默认带有开头/@/views" />
        </el-form-item>
        <el-row :gutter="16">
        ...
        <el-row :gutter="16">
          <el-col :span="12">
            <el-form-item label="接口权限(saAuth模式)" prop="permsList">
              <el-select v-model="form.permsList" :multiple="true" style="width: 100%" placeholder="请选择接口权限">
                <el-option v-for="item in allUrlData" :key="item.name" :value="item.name" :label="item.url">
                  {{ item.url }}
                </el-option>
              </el-select>
            </el-form-item>
