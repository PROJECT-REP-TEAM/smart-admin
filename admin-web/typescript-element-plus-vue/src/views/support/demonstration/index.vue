<template>
  <div>
    <el-card title="地区选择" class="smart-margin-top10">
      <smart-area-cascader type="province_city_district" :width="200" v-model:value="area" @change="changeArea" />
    ...
    <el-card title="下拉组件" class="smart-margin-top10">
      <el-row class="smart-margin-top10">
        <el-col :span="12">
          枚举选择:
          <smart-enum-select :width="200" v-model:value="goodsType" placeholder="请选择类型" enum-name="GOODS_TYPE_ENUM" />
        </el-col>
        <el-col :span="12"> 值：{{ goodsType }} </el-col>
      </el-row>
      <el-row class="smart-margin-top10">
        <el-col :span="12">
          真假选择:
          <smart-boolean-select :width="200" v-model:value="trueFalse" placeholder="请选择" />
        </el-col>
        <el-col :span="12"> 值：{{ trueFalse }} </el-col>
      </el-row>
    </el-card>
    ...
    <el-card title="富文本编辑器" class="smart-margin-top10">
      ...
      <div class="smart-margin-top10">
        <textarea v-model="content" readonly style="width: 100%; height: 200px; outline: none"></textarea>
      </div>
    </el-card>
  </div>
</template>
...
<script setup lang="ts">
  ...
  let selectArea = reactive<AreaVo>({});
  let goodsType = ref();
  let trueFalse = ref();
  ...
  function changeArea(value: number[]) {
    if (!_.isEmpty(value)) {
      // 地区信息
