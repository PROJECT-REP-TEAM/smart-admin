<template>
  <el-drawer :size="1300" v-model="visible" title="字典值" @close="onClose">
    ...
    <el-card shadow="hover">
      <el-row class="smart-table-btn-block">
        <div class="smart-table-operate-block">
          <el-button @click="addOrUpdateValue" type="primary" size="small" icon="Plus"> 新建 </el-button>
...
<script setup lang="ts">
  ...
  const dictKeyId = ref<number>();
  // ----------------------- 以下是生命周期 ------------------------

  // ----------------------- 以下是方法 ------------------------
  function showModal(keyId: number) {
  ...
  const queryFormState: DictValueQueryForm = {
    dictKeyId: undefined,
    searchWord: '',
    pageNum: 1,
    pageSize: 10,
  };
  const queryForm = reactive<DictValueQueryForm>({ ...queryFormState });
  const rowKey = ref('dictValueId');
  const selectedRowKeys = ref<number[]>([]);
  const selectionRows = ref<DictValueVo[]>([]);
  const tableLoading = ref(false);
  const tableData = ref<DictValueVo[]>([]);
  const total = ref(0);

  // 选择行
  async function onSelect(selection: DictValueVo[]) {
    // 找到当页未被勾选项
