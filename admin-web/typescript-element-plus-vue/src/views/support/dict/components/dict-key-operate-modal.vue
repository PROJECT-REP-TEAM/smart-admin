<template>
  <el-drawer v-model="visible" :title="form.dictKeyId ? '编辑' : '添加'" @close="onClose">
...
<script setup lang="ts">
  ...
  const formDefault: DictKeyAddForm & DictKeyUpdateForm = {
    dictKeyId: undefined,
    keyCode: '',
    keyName: '',
    remark: '',
  };
  let form = reactive<DictKeyAddForm & DictKeyUpdateForm>({ ...formDefault });
  const rules = {
  ...
  // ----------------------- 以下是方法 ------------------------
  function showModal(rowData?: DictKeyUpdateForm) {
