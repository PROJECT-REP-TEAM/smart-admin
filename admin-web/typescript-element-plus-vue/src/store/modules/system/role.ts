 ...
 * @LastEditTime: 2021-08-28 16:37:27
 * @LastEditors: zhuoda
 * @Description:
...
export const useRoleStore = defineStore({
  ...
  state: (): {
    checkedData: number[];
    treeMap: Map<number, MenuSimpleTreeVo>;
  } => ({
    checkedData: [],
    treeMap: new Map(),
  }),
  ...
  actions: {
    // 初始化权限树选中数据
    initCheckedData(data: number[]) {
      this.checkedData = [...new Set(data)];
    },
    // 选中
    addCheckedData(data: number) {
    ...
    // 选中本级以及子级
    addCheckedDataAndChildren(data: MenuSimpleTreeVo) {
    ...
    // 取消选中
    deleteCheckedData(index: number) {
      this.checkedData.splice(index, 1);
    },
    // 取消选中本级以及子级
    deleteCheckedDataAndChildren(data: MenuSimpleTreeVo) {
      let findIndex = this.checkedData.findIndex((val) => val == data.menuId);
      if (findIndex != -1) {
    ...
    // 初始化权限树对象
    initTreeMap(tree: MenuSimpleTreeVo[]) {
    ...
    // 选中上一级
    selectUpperLevel(module: MenuSimpleTreeVo) {
      // 拿到上级key
