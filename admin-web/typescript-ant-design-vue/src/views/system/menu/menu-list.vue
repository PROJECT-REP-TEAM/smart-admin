...
async function query() {
  try {
    ...
    let responseModel: ResponseModel<MenuVo[]> = await menuApi.queryMenu();
    // 过滤搜索条件
...
let selectedRows: MenuVo[] = [];
const hasSelected = computed(() => selectedRowKeys.value.length > 0);
function onSelectChange(keyArray: Key[], selectRows: MenuVo[]): void {
