 ...
 * @LastEditTime: 2021-09-01 20:58:27
 * @LastEditors: zhuoda
    ...
    <a-form ref="formRef" :model="form" :rules="rules" layout="vertical">
      ...
      <a-form-item label="菜单类型" name="menuType">
        <a-radio-group v-model:value="form.menuType">
          <a-radio v-for="item in MENU_TYPE_ENUM" :key="item.value" :value="item.value">{{ item.desc }}</a-radio>
        </a-radio-group>
      </a-form-item>
      <!--      目录 菜单 start   -->
        ...
        <a-row :gutter="16">
          ...
          <a-col :span="12">
            <a-form-item label="排序" name="sort">
              <a-input-number style="width: 100%" placeholder="请输入排序" v-model:value="form.sort" :min="0" :max="9999" />
            </a-form-item>
          </a-col>
        </a-row>
        <a-form-item v-if="form.menuType == MENU_TYPE_ENUM.MENU.value" label="组件地址" name="component">
        ...
        <a-row :gutter="16">
          <a-col :span="12">
            <a-form-item label="接口权限" name="permsList">
              ...
              <a-select v-model:value="form.permsList" mode="multiple" style="width: 100%" placeholder="请选择接口权限">
