...
<template>
  <el-card title='角色列表' class='role-container' style='padding: 0'>
    <template #header>
      <el-button v-privilege="'system:role:add'" type='primary' size='small' @click='showOperateRoleModal'>添加
      </el-button>
    </template>
          ...
          <el-popover placement='right' trigger='hover'>
            <div class='btn-group'>
              <div class='btn' v-privilege="'system:role:delete'">
                <el-button link type='primary' @click='deleteRole(item.roleId)'>删除
                </el-button>
              </div>
              <div class='btn' v-privilege="'system:role:update'">
                <el-button link type='primary' @click='showOperateRoleModal(item)'>编辑
                </el-button>
              </div>
            </div>
            <template #reference>
