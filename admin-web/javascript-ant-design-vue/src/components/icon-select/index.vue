  ...
  <div>
    <a-popover v-model:visible="visible" placement="bottomLeft" trigger="click">
      ...
      <template #content>
        <div class="icon-box">
          <div
            v-for="(item, index) in iconLoopArray"
            :key="item"
          ...
          <div v-show="showMoreIndex > 0">
            <a-button type="link" @click="showMore"
              >点击展开更多图标（因图标较多，可能会卡一小会）</a-button
            >
          </div>
        </div>
      </template>
...
//线框风格图标数组
const outlinedIconArray = Object.keys(VueIcon).filter((e) =>
  _.endsWith(e.toLowerCase(), "outlined")
);
//实底风格图标数组
const filledIconArray = Object.keys(VueIcon).filter((e) =>
  _.endsWith(e.toLowerCase(), "filled")
);
...
function updateSelectIconArray() {
  ...
  if (!searchValue.value) {
    ...
    selectIconArray.value = tempArray.filter((e) =>
